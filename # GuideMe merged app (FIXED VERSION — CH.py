# GuideMe merged app (FIXED VERSION — CHUNK 1 + 2)
...
def estimate_travel_time_km(distance_km, transport):
    ...
    hours = distance_km / s
    if transport == "Train": hours *= 1.15
    if transport == "Bus": hours *= 1.2
    return hours
...
class LoginPage(ctk.CTkFrame):
    def __init__(self, master):
        ...
        # white card — FIX: move width & height INTO constructor
...
class SignupModal(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        # FIX: card width/height moved inside constructor
...
class TripPage(ctk.CTkFrame):
    ...
    def build_plan_trip(self):
        ...
        for t in ["Flight", "Train", "Bus", "Car"]:
            btn = ctk.CTkButton(trans_frame, text=t, width=70,
                                ...
                                hover_color="#D6F3EE",
                                command=lambda tt=t: self.select_transport(tt))
            btn.pack(side="left", padx=4)
            self.transport_buttons[t] = btn

        # INFO LABEL
        self.info_label = ctk.CTkLabel(tab, text="Distance: -- km\nTime: --")
        self.info_label.pack(pady=10)

        # Generate Button
        ctk.CTkButton(tab, text="Generate Packages",
                      fg_color=PALETTE["accent"],
                      command=self.generate_packages).pack(pady=10)
    ...
    def show_suggestions(self, trip_type):
        ...
        for d in DESTINATION_SUGGESTIONS.get(trip_type, []):
            ctk.CTkButton(self.suggest_frame, text=d, width=80,
                          fg_color="#E6F9F6",
                          command=lambda x=d: self.dest_entry.delete(0, "end") or self.dest_entry.insert(0, x)
    ...
    def compute_route(self):
        ...
        self.distance_km = round(haversine_km(a, b), 1) if (a and b) else random.randint(80, 600)
        self.travel_hours = estimate_travel_time_km(self.distance_km, self.transport)
